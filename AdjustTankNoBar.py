...
def move_tank_value(direction, Colorstate):
    ...
    if direction > 0:
        # 6.99 ist 0.5 cm
        drive_tank.on_for_degrees(SpeedPercent(DRIVE_SPEED), SpeedPercent(DRIVE_SPEED), TURN_DEGREE)  # vorwärts
    else:
        drive_tank.on_for_degrees(SpeedPercent(DRIVE_SPEED), SpeedPercent(DRIVE_SPEED), 1.5 * -TURN_DEGREE)  # rückwärts
    return False
